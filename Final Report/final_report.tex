...
\begin{center}
    ...
    {\small VinUniversity, College of Engineering and Computer Science}\\[2pt]
...
%==================== ABSTRACT ====================
\noindent\textbf{Abstract.}
\textit{Hanoi Air Quality Pulse} is a deployed Python Shiny dashboard for exploring Hanoi's air pollution
through four coordinated pages (Overview, Districts, History, Forecast) built on a consistent visual
language. A single citywide AQI hides variation by district, hour, season, pollutant, and weather; our
design pairs encodings to specific analytical questions and uses interaction to turn passive charts into
exploratory tools: linked district selection drives four views at once, a reveal-on-demand anomaly toggle
preserves analytical honesty, and 3D/2D map alternatives balance storytelling against accessibility.
Short-term forecasts are embedded as an interpretable complement to the visualization, backed by historical
and live data with cached ML models.
...
Three goals drive every design choice: \textbf{(1)~interpret the present} for non-experts,
\textbf{(2)~enable comparison} across 30 districts and time scales, and \textbf{(3)~embed short-term
forecasts} as chart extensions, not detached outputs. Four tabs answer: current conditions (Overview),
district rankings and shifts (Districts), temporal patterns and anomalies (History), and short-term risk
(Forecast).

%==================== 2. VISUALIZATION & INTERACTION DESIGN ====================
\section{Visualization and Interaction Design}
This section is the core of the project: each visualization is presented as a design decision, namely the
encoding chosen, the question it answers, and the interaction that makes it explorable.

\noindent \textbf{A consistent perceptual language.}
...
\caption*{24\,h line (observed vs.\ 1\,h forecast), exposure card, and 3D map.}
\end{subfigure}
...
exact rank and value in a sortable table, with twelve month columns coloured by the AQI scale. It is chosen
...
% \caption{Monthly trend and pollutant breakdown driven by the same selection.}
% \end{subfigure}
% \\[2pt]
% \begin{subfigure}{0.6\textwidth}\centering
% \includegraphics[width=\linewidth]{fig_districts_1.jpg}
% \caption{Linked choropleth, ranking table, and district presets.}
% \end{subfigure}
% \caption{Districts page: coordinated multiple views.}
% \label{fig:districts}
% \end{figure}
...
\caption{Calendar heatmap, seasonal overlay, and anomaly toggle.}
\end{subfigure}
\end{figure}
...
\caption{Category donut and month$\times$year stacked-bar matrix.}
\end{subfigure}
...
data-quality events, giving analytical honesty without clutter.
...
% \caption{Forecast hero, source-tier chip, and Key-Drivers radar.}
% \end{subfigure}
% \\[2pt]
% \begin{subfigure}{0.6\textwidth}\centering
% \includegraphics[width=\linewidth]{fig_forecast_2.jpg}
% \caption{Model-vs-baseline errors, PCA learning space, and backtest line.}
% \end{subfigure}
% \caption{Forecast page with interpretability views.}
% \label{fig:forecast}
% \end{figure}
...
\noindent \textbf{Anomaly-aware cleaning that protects the visuals.}
...
\noindent \textbf{Realtime architecture (Figure~\ref{fig:arch}).}
...
\noindent The dashboard shows baseline error alongside model error precisely so this nuance is visible rather than
...
grow with horizon, where persistence weakens; the $-1.4\%$ at PM2.5 1h is an honest limitation, since for
very short-term PM2.5 the baseline is already strong.}
\label{tab:model}
\end{table}

%==================== 6. FINDINGS, LIMITATIONS, FUTURE WORK ====================
\section{Findings, Limitations, and Future Work}
\noindent \textbf{Findings.} The coordinated design surfaces patterns a single AQI number cannot: clear diurnal and
...
\noindent \textbf{Limitations.} Realtime station coverage can be sparse or stale, and the crawler currently stores
...
\noindent \textbf{Future work.} Store richer realtime weather/pollutant features, add uncertainty bands to forecasts,
